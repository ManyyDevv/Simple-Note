...
if(!localStorage.getItem('sn4_admin_pw')) localStorage.setItem('sn4_admin_pw', 'admin2026');
...
function bindAdmin(){
  document.getElementById('admin-entry-btn').onclick = () => {
    ...
    document.getElementById('admin-content').classList.toggle('hidden', !adminUnlocked);
    if(adminUnlocked) renderAdminAll();
    else setTimeout(()=>document.getElementById('admin-pw-input')?.focus(), 50);
  };
  document.getElementById('admin-unlock-btn').onclick = tryAdminUnlock;
  document.getElementById('admin-pw-input').addEventListener('keydown', e => { if(e.key==='Enter') tryAdminUnlock(); });
...
function tryAdminUnlock(){
  ...
  const stored = localStorage.getItem('sn4_admin_pw');
  if(pw === stored){
    adminUnlocked = true;
    document.getElementById('admin-gate').classList.add('hidden');
    document.getElementById('admin-content').classList.remove('hidden');
    document.getElementById('admin-pw-error').classList.add('hidden');
    logActivity('Connexion administrateur');
    renderAdminAll();
  } else {
...
function changeAdminPassword(){
  ...
  if(cur !== stored){ err.classList.remove('hidden'); return; }
  if(!next){ return; }
  localStorage.setItem('sn4_admin_pw', next);
  err.classList.add('hidden');
  document.getElementById('admin-pw-current').value = '';
  document.getElementById('admin-pw-new').value = '';
  logActivity('Mot de passe administrateur modifié');
