...
// Adresses email considérées comme "toi" (propriétaire du site) : elles seules
// voient l'onglet Modération et peuvent supprimer n'importe quelle publication
// du Hub. Ajoute d'autres emails séparés par une virgule si besoin.
const ADMIN_EMAILS = ['vibeonchill79@gmail.com'];
function isAdmin(){ return !!currentUser && ADMIN_EMAILS.includes((currentUser.email||'').toLowerCase()); }

let currentGlobalView = 'accueil'; // 'accueil' | 'hub' | 'notes'
...
async function handleAuthChange(user){
  document.getElementById('app-loading')?.classList.add('hidden');
  document.getElementById('global-topnav').classList.remove('hidden');

  if(user){
    currentUser = user;
    document.getElementById('current-user-email').textContent = user.email;
    document.getElementById('gtn-user-email').textContent = user.email;
    document.getElementById('gtn-user-email').classList.remove('hidden');
    document.getElementById('gtn-signout-btn').classList.remove('hidden');
    document.getElementById('nav-moderation-btn').classList.toggle('hidden', !isAdmin());
    ...
    showGlobalView('notes');
  } else {
    currentUser = null;
    notes = []; folders = [];
    document.getElementById('gtn-user-email').classList.add('hidden');
    document.getElementById('gtn-signout-btn').classList.add('hidden');
    document.getElementById('nav-moderation-btn').classList.add('hidden');
    showGlobalView('accueil');
  }
}

// ════════════════════════════════════
//  NAVIGATION GLOBALE (Accueil / Hub / Notes)
//  C'est le niveau le plus haut du site : ces 3 écrans existent que tu
//  sois connecté ou non. "Notes" est ton espace personnel (protégé),
//  "Hub" est public (tout le monde peut y jeter un œil), "Accueil" est
//  la page de présentation.
// ════════════════════════════════════
function bindGlobalNav(){
  document.querySelectorAll('.gtn-tab').forEach(btn => {
    btn.addEventListener('click', () => showGlobalView(btn.dataset.view));
  });
  document.getElementById('gtn-signout-btn').addEventListener('click', () => window.fbApi.signOutUser());
}
function showGlobalView(view){
  if(view==='notes' && !currentUser){
    // Pas connecté et on demande "Notes" → on ouvre direct le formulaire de connexion
    showAuthGate('signin');
    return;
  }
  currentGlobalView = view;
  document.querySelectorAll('.gtn-tab').forEach(b => b.classList.toggle('active', b.dataset.view===view));
  document.getElementById('landing').classList.toggle('hidden', view!=='accueil');
  document.getElementById('auth-gate').classList.add('hidden');
  document.getElementById('hub-shell').classList.toggle('hidden', view!=='hub');
  document.getElementById('app-shell').classList.toggle('hidden', view!=='notes');
...
function bindLanding(){
  document.querySelectorAll('.landing-start-btn').forEach(b => b.addEventListener('click', () => {
    currentUser ? showGlobalView('notes') : showAuthGate('signup');
  }));
  document.getElementById('auth-back-btn').addEventListener('click', () => showGlobalView('accueil'));
}
function showAuthGate(mode){
  document.querySelectorAll('.gtn-tab').forEach(b => b.classList.remove('active'));
  document.getElementById('landing').classList.add('hidden');
  document.getElementById('hub-shell').classList.add('hidden');
...
function goToPage(name){
  ...
  if(name==='moderation'){ if(isAdmin()) renderModeration(); else goToPage('dashboard'); }
...
/* ════════════════════════════════════════════════════════════
   ...
   Explication simple : le Hub est une grande salle commune sur le
   serveur (une collection Firestore "hub_posts") que TOUT LE MONDE
   peut lire, même sans être connecté. Quand tu "publies" une note,
   on en dépose une copie dans cette salle. Chacun peut ensuite la
   "liker" (son uid est ajouté à la liste "likedBy" de la
   publication) ou la signaler si elle pose problème.
════════════════════════════════════════════════════════════ */
let hubCache = [];
let hubSort  = 'recent';

function bindHub(){
  document.getElementById('hub-search').addEventListener('input', renderHubGrid);
  document.querySelectorAll('.hub-sort-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      hubSort = btn.dataset.sort;
      document.querySelectorAll('.hub-sort-btn').forEach(b => b.classList.toggle('active', b===btn));
      renderHubGrid();
    });
  });
  document.getElementById('hub-login-hint-btn').addEventListener('click', () => showAuthGate('signin'));
  document.getElementById('hub-publish-btn').addEventListener('click', () => {
    if(!currentUser) return showAuthGate('signin');
    showGlobalView('notes');
    goToPage('home');
    showToast('👉 Ouvre une note puis clique sur « Publier au Hub »');
  });
}

async function renderHub(){
  document.getElementById('hub-publish-btn').classList.toggle('hidden', !currentUser);
  document.getElementById('hub-login-hint').classList.toggle('hidden', !!currentUser);
  const grid = document.getElementById('hub-grid');
  grid.innerHTML = '<div class="hub-empty">Chargement…</div>';
  try{
    hubCache = await window.fbApi.fetchHubPosts();
  }catch(e){
    grid.innerHTML = '<div class="hub-empty">Impossible de charger le Hub pour le moment. Réessaie plus tard.</div>';
    return;
  }
  renderHubGrid();
}
...
function renderHubGrid(){
  ...
  if(hubSort==='popular') list.sort((a,b) => (b.likedBy?.length||0) - (a.likedBy?.length||0));
  else list.sort((a,b) => new Date(b.createdAt) - new Date(a.createdAt));

  if(!list.length){
    grid.innerHTML = '<div class="hub-empty">Aucune note publiée pour l\'instant. Sois le premier à en partager une !</div>';
    return;
  }

  grid.innerHTML = list.map(p => {
    const liked = !!(currentUser && (p.likedBy||[]).includes(currentUser.uid));
    const mine  = !!(currentUser && p.authorUid===currentUser.uid);
    return `
    <div class="hub-card">
      <div class="hub-card-top">
        <span class="hub-card-dot" style="background:${COLOR_HEX[p.color]||COLOR_HEX.gold}"></span>
        <span class="hub-card-title">${esc(p.title||'Sans titre')}</span>
      </div>
      <p class="hub-card-desc">${esc(p.description||stripTags(p.content||'').slice(0,140))}</p>
      ${(p.tags||[]).length ? `<div class="hub-card-tags">${p.tags.slice(0,4).map(t=>`<span class="ntag" style="--tc:${tagColor(t)}">${esc(t)}</span>`).join('')}</div>` : ''}
      <div class="hub-card-author">par ${esc((p.authorEmail||'anonyme').split('@')[0])}</div>
      <div class="hub-card-ft">
        <button class="hub-like-btn ${liked?'liked':''}" onclick="toggleHubLike('${p.hubId}')">
          ${liked ? '❤️' : '🤍'} ${(p.likedBy||[]).length}
        </button>
        <button class="icon-pill" onclick="readHubPost('${p.hubId}')">Lire</button>
        ${mine
          ? `<button class="icon-pill" onclick="deleteMyHubPost('${p.hubId}')">Supprimer</button>`
          : `<button class="icon-pill" onclick="reportHubPost('${p.hubId}')" title="Signaler">🚩</button>`}
      </div>
    </div>`;
  }).join('');
}

function readHubPost(hubId){
  const p = hubCache.find(x => x.hubId===hubId);
  if(!p) return;
  document.getElementById('d-title').textContent   = p.title || 'Sans titre';
  document.getElementById('d-author').textContent  = p.author || (p.authorEmail||'').split('@')[0];
  document.getElementById('d-date').textContent     = fmtDate(p.createdAt);
  document.getElementById('d-words').textContent    = countWords(p.content||'')+' mots';
  document.getElementById('d-time').textContent     = Math.max(1, Math.round(countWords(p.content||'')/200))+' min';
  document.getElementById('d-folder-badge').innerHTML = '';
  document.getElementById('d-tags').innerHTML = (p.tags||[]).map(t=>`<span class="ntag" style="--tc:${tagColor(t)}">${esc(t)}</span>`).join('');
  document.getElementById('d-content-render').innerHTML = p.content || '<p><em>Aucun contenu.</em></p>';
  document.getElementById('detail-card').querySelector('.detail-hd')?.classList.add('hidden');
  currentId = null; // ce n'est pas une note personnelle, on bloque les actions d'édition
  goToPage('detail');
}

function toggleHubLike(hubId){
  if(!currentUser) return showAuthGate('signin');
  const p = hubCache.find(x => x.hubId===hubId);
  if(!p) return;
  const liked = (p.likedBy||[]).includes(currentUser.uid);
  p.likedBy = liked ? p.likedBy.filter(u=>u!==currentUser.uid) : [...(p.likedBy||[]), currentUser.uid];
  renderHubGrid();
  window.fbApi.toggleHubLike(hubId, currentUser.uid, liked).catch(() => showToast('❌ Impossible d\'enregistrer ton like, réessaie.'));
}

function reportHubPost(hubId){
  if(!currentUser) return showAuthGate('signin');
  if(!confirm('Signaler cette note aux personnes qui gèrent le site ?')) return;
  window.fbApi.reportHubPost(hubId)
    .then(() => showToast('🚩 Note signalée, merci !'))
    .catch(() => showToast('❌ Erreur, réessaie.'));
}

function deleteMyHubPost(hubId){
  if(!confirm('Retirer définitivement cette note du Hub ?')) return;
  window.fbApi.deleteHubPost(hubId)
    .then(() => { hubCache = hubCache.filter(p => p.hubId!==hubId); renderHubGrid(); showToast('🗑️ Note retirée du Hub.'); })
    .catch(() => showToast('❌ Erreur, réessaie.'));
}

function publishCurrentNoteToHub(){
  const note = notes.find(n => n.id===currentId);
  if(!note || !currentUser) return;
  if(note.locked) return showToast('🔒 Impossible de publier une note verrouillée par mot de passe.');
  if(!confirm('Publier « '+note.title+' » sur le Hub public ? Tout le monde pourra la lire.')) return;
  const payload = { title:note.title, description:note.description, author:note.author, content:note.content, tags:note.tags, color:note.color };
  window.fbApi.publishToHub(currentUser.uid, currentUser.email, payload)
    .then(() => showToast('🌐 Note publiée sur le Hub !'))
    .catch(() => showToast('❌ Impossible de publier, réessaie.'));
}

/* ════════════════════════════════════════════════════════════
   MODÉRATION (visible seulement pour ADMIN_EMAILS)
════════════════════════════════════════════════════════════ */
function bindModeration(){
  document.getElementById('moderation-refresh-btn').addEventListener('click', renderModeration);
}
async function renderModeration(){
  ...
  let posts;
  try{
    posts = await window.fbApi.fetchHubPosts();
  }catch(e){
    tbody.innerHTML = `<tr><td colspan="6">Impossible de charger le Hub.</td></tr>`;
    return;
  }
  posts.sort((a,b) => (b.reported?1:0)-(a.reported?1:0) || new Date(b.createdAt)-new Date(a.createdAt));

  document.getElementById('moderation-stats').innerHTML = [
    {lbl:'Publications', val:posts.length},
    {lbl:'Signalées', val:posts.filter(p=>p.reported).length},
    {lbl:'Likes au total', val:posts.reduce((s,p)=>s+(p.likedBy?.length||0),0)},
  ...
  badge.classList.toggle('hidden', reportedCount===0);

  tbody.innerHTML = posts.length ? posts.map(p => `
    <tr>
      <td>${esc(p.title||'Sans titre')}</td>
      <td>${esc(p.authorEmail||'—')}</td>
      <td>${(p.likedBy||[]).length}</td>
      <td>${p.reported ? '🚩 Oui' : '—'}</td>
      <td>${fmtDate(p.createdAt)}</td>
      <td><button class="btn-danger" onclick="adminDeleteHubPost('${p.hubId}')">Supprimer</button></td>
    </tr>`).join('') : `<tr><td colspan="6">Aucune publication pour l'instant.</td></tr>`;
}
function adminDeleteHubPost(hubId){
  if(!confirm('Supprimer définitivement cette publication du Hub ?')) return;
  window.fbApi.deleteHubPost(hubId).then(renderModeration).catch(() => showToast('❌ Erreur, réessaie.'));
}
